...
@implementation ExpandableClearBar
...
{
    if ([self clearDelegate] && [[self clearDelegate] respondsToSelector:@selector(expandingBarWillAppear:)]) {
        [[self clearDelegate] expandingClearBarWillAppear:self];
    }
    ...
    if ([self clearDelegate] && [[self clearDelegate] respondsToSelector:@selector(expandingBarDidAppear:)]) {
        [[self clearDelegate] performSelector:@selector(expandingBarDidAppear:) withObject:self afterDelay:delegateDelay];
    }
}

- (void) hideButtonsAnimated:(BOOL)animated
{
    if ([self clearDelegate] && [[self clearDelegate] respondsToSelector:@selector(expandingBarWillDisappear:)]) {
        [[self clearDelegate] performSelector:@selector(expandingBarWillDisappear:) withObject:self];
    }
    ...
    float delegateDelay = _animated ? [[self buttons] count] * _delay + _animationTime: 0.0f;
    if ([self clearDelegate] && [[self clearDelegate] respondsToSelector:@selector(expandingBarDidDisappear:)]) {
        [[self clearDelegate] performSelector:@selector(expandingBarDidDisappear:) withObject:self afterDelay:delegateDelay];
    }
    _toggled = YES;
    [self toggleMainButton];
}
